...
@router.message(AnonymousSendStates.waiting_message, F.content_type.in_({"text", "photo", "voice"}))
async def handle_anonymous_content(message: Message, session: AsyncSession, state: FSMContext, bot: Bot, rate_limited: bool = False) -> None:
    ...
    message_type, text_content, file_id, voice_duration, error_key = _extract_content(message)
    if error_key:
        await message.answer(t(error_key, lang))
        return

    antispam = AntiSpamService(session)
    dup = await antispam.check_duplicate(message.from_user.id, receiver_id, text_content)
    if not dup.allowed:
    ...
    if delivered is not None:
        await messages_service.link_delivered_message(stored, delivered.message_id)

    await message.answer(t("message_delivered", lang))
    await state.clear()


def _extract_content(message: Message) -> tuple[MessageType | None, str | None, str | None, int | None, str | None]:
    if message.text:
    ...
    if message.photo:
        ...
        return MessageType.PHOTO, message.caption, largest.file_id, None, None
    ...
    if message.voice:
        ...
        return MessageType.VOICE, None, message.voice.file_id, message.voice.duration, None
...
async def _deliver_to_receiver(
    ...
    kb = received_message_kb(receiver_lang, stored_message_id, can_reveal)
    header = t("new_anonymous_message", receiver_lang)
    try:
        if message_type == MessageType.TEXT:
            body = escape_html(text_content or "")
            return await bot.send_message(receiver_telegram_id, f"{header}\n\n{body}", parse_mode="HTML", reply_markup=kb)
        if message_type == MessageType.PHOTO:
            caption = f"{header}" + (f"\n\n{escape_html(text_content)}" if text_content else "")
            return await bot.send_photo(receiver_telegram_id, file_id, caption=caption, parse_mode="HTML", reply_markup=kb)
        if message_type == MessageType.VOICE:
            return await bot.send_voice(receiver_telegram_id, file_id, caption=header, parse_mode="HTML", reply_markup=kb)
    except TelegramAPIError as exc:
        logger.warning("Failed to deliver anonymous message to %s: %s", receiver_telegram_id, exc)
    return None
