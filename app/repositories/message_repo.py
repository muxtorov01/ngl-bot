...
class MessageRepository:
    ...
    async def get_or_create_thread(self, receiver_id: int, sender_telegram_id: int) -> MessageThread:
        ...
        if thread:
            return thread
        thread = MessageThread(receiver_id=receiver_id, sender_telegram_id=sender_telegram_id)
    ...
    async def set_delivered_chat_message_id(self, message: Message, chat_message_id: int) -> None:
        message.delivered_chat_message_id = chat_message_id
        await self.session.flush()

    async def get_by_delivered_chat_message_id(self, receiver_id: int, chat_message_id: int) -> Message | None:
        result = await self.session.execute(
    ...
    async def recent_identical_count(
        self, sender_telegram_id: int, receiver_id: int, text_content: str, since: dt.datetime
    ) -> int:
        result = await self.session.execute(
    ...
    async def count_since(self, sender_telegram_id: int, since: dt.datetime) -> int:
        result = await self.session.execute(
            select(func.count(Message.id)).where(
                Message.sender_telegram_id == sender_telegram_id,
